...
methods (Access = protected)
    ...
    % Evaluate the function at (numeric) places
        val = cos(this.Inputs.eval()) ;
    end
end

methods
    function fcn = diff(this,obj)
    % Return the function derivative w.r.t an input (or variable)
        if nargin<2 ; obj = this.Variables(1) ; end
        if obj==this % differentiation wrt itself=1
            fcn = pkg.sym.One(obj) ;
        else % chain rule
            fcn = -pkg.sym.Sin(this.Inputs)*diff(this.Inputs,obj) ;
        end
    end
end
