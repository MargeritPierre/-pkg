...
% Grid creation
grid = pkg.fft.Grid(N,'dx',L./N) ;
clf ; axis equal ; plot(grid) ;

% Grid composition
xgrid = pkg.fft.Grid([N 1],'dx',L(1:2)./N) ;
ygrid = pkg.fft.Grid([1 N],'dx',L(1:2)./N) ;
tp_grid = xgrid.*ygrid ;
...
% Pressure field
p = pkg.fft.Variable(grid,'Order',0) ;
% Displacement field
u = pkg.fft.Variable(grid,'Order',1) ;
% Strain field
EPS = pkg.fft.Variable(grid,'Order',2) ;
% Stiffness tensor field
C = pkg.fft.Variable(grid,'Order',4) ;
% [displacement+rotation] field
TORS = pkg.fft.Variable(grid,'Size',[3 2]) ;
...
% Any other function
kappa = pkg.fft.Variable(grid).setdata(@rand) ;
mu = pkg.fft.Variable(grid).setdata(@rand) ;
...
f = pkg.fft.Variable(grid,'Order',0) ;
k = 2*pi./L ;
f.Data = reshape(exp(1i*k*x.Data(:,:)),f.Grid.N) ;
gradF = pkg.fft.operators.nabla(fft(f)) ;
gradF.apply() ;
laplacianF = pkg.fft.operators.nabla(gradF.Outputs,[],true) ;
laplacianF.apply() ;
