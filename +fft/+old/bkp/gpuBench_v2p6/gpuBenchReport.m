...
[textProgess,graphicalProgress] = gpubench.getDisplayMode();
...
totaltasks = N+3;
tasksDone = 0;
title = sprintf('Creating GPUBench v%s report', gpubench.version);
if textProgess
    fprintf( [title,' .'] );
end
if graphicalProgress
    ...
    drawnow(); % Make sure the progressbar appers straight away.
else
    wb = [];
end

% Ensure the output folder exists
reportDir = tempname(fullfile(tempdir(),'GPUBenchReport'));
if ~exist( reportDir, 'dir' )
    mkdir( reportDir );
end
copyFiles( reportDir );
tasksDone = iShowProgress(graphicalProgress, textProgess, tasksDone, totaltasks, wb);
...
bars = barh( flopsResults'/1e12 );
set( gca, ...
    'YDir', 'Reverse', ...
    'YTickLabel', functionNames, 'TickLength', [0 0])
xlabel( sprintf('TFLOPS\n(higher is better)') )
...
% Add faint dividers between benchmarks
hold on
x = get(gca,'XLim');
minorGridColor = [get(gca,'MinorGridColor') get(gca,'MinorGridAlpha')];
for ii=1:size(flopsResults,2)-1
    plot( x, (ii+0.5)*[1 1], '-', ...
        'Color', minorGridColor, 'HandleVisibility', 'off');
end
...
for rr=1:numel( results )
    ...
    [maxVal,maxIdx] = max( 1e-12 * thisResult.NumOps ./ thisResult.Times);
...
for ii=1:numel(types)
    fprintf( fid, '        <th data-type="nosort" class="summarytable" colspan="%d">%s precision results (in TFLOPS)</th>\n', numOfType(ii), strcat(upper(types{ii}(1)), types{ii}(2:end)) );
end
fprintf( fid, '      </tr>\n' );
fprintf( fid, '      <tr class="summarytable">\n' );
fprintf( fid, '        <th data-type="nosort" class="summarytable"></th>\n' );
...
for rr=1:numel(summaryData.DeviceName)
    ...
    if (summaryData.IsSelectedDevice(rr))
        ...
        cellformat = '        <td class="summarytable" align="right"><a href="device%d.html#result%u"><b>%1.3f</b></a></td>\n';
    else
        nameStr = summaryData.DeviceName{rr};
        cellformat = '        <td class="summarytable" align="right"><a href="device%d.html#result%u">%1.3f</a></td>\n';
    end
    fprintf( fid, '        <td class="summarytable" align="left"><a href="device%d.html">%s</a></td>\n', rr, nameStr );
    for cc=1:numCols
        colIdx = colOrder(cc);
        fprintf( fid, cellformat, rr, colIdx, summaryData.PeakFLOPS(rr,colIdx) / 1e12 );
    end
    fprintf( fid, '      </tr>\n' );
end
fprintf( fid, '    </tbody>\n' );
...
for nn=1:numel( names )
    ...
    fprintf( fid, '                  <th class="summarytable">TeraFLOPS</th>\n' );
    fprintf( fid, '                </tr>\n' );
    ...
    for ss=1:numel(sizes)
        ...
        fprintf( fid, [formatStr1,'%2.3f',formatStr2], times(ss)*1000 );
        fprintf( fid, [formatStr1,'%2.3f',formatStr2], flops(ss)/times(ss)/1e12 );
        fprintf( fid, '                </tr>\n' );
    end
    fprintf( fid, '              </table>\n' );
    fprintf( fid, '              <center><small>(<code>N</code> teraflops = <code>Nx10<sup>12</sup></code> operations per second)</small></center><br/>\n' );
    fprintf( fid, '            </td>\n');
    fprintf( fid, '          </tr>\n');
    fprintf( fid, '        </table>\n');
    fprintf( fid, '      </td>\n');
...
%-------------------------------------------------------------------------%
function idx = iShowProgress(showWaitbar, showText, idx, numTasks, figh)
idx = idx + 1;
if showWaitbar
    if isvalid(figh)
        ...
        error('gpuBenchReport:Cancel', 'User cancelled')
    end
end
if showText
    fprintf( '.' );
