...
% GRID A Uniform grid
    ...
    methods
        ...
        function D = ndims(this) ; D = numel(this.N) ; end

        function idx = dataindices(this)
            idx = cellfun(@colon,num2cell(0*this.N,1),num2cell(this.N-1,1),'uni',false) ; % arrayfun "uni,false" does not support gpuArrays
            [idx{:}] = ndgrid(idx{:}) ;
            idx = cat(ndims(this)+1,idx{:}) ;
            idx = pkg.fft.Field(this,'Order',1,'Data',idx) ;
        end

        function x = coordinates(this)
            x = copy(this.dataindices()) ;
            x.Data = this.x0.' + reshape(x.Data,prod(this.N),[])*(this.dx.') ;
        end
        ...
        function pl = plot(this,varargin)
            ...
            switch this.ndims
                ...
                case 2
                    pl = surf(x(:,:,1),x(:,:,2),zeros(this.N),NaN(this.N),varargin{:}) ;
                case 3
                otherwise
            end
        end
