...
%% FREEFORM
    ...
    t = .25 ; lvlst = pkg.geometry.levelset.Freeform(@(p)abs(fcn(p))-sqrt(3)*t,bbox) ;
...
%% BUILD A MESH
    ...
    profile on
    if lvlst.nCoord==3 
        disp('mesh boundary surface')
        pfix = pkg.geometry.mesh.distMesh(args{:},'bnd_only',true).Nodes ; 
        args = [args {'pfix',pfix}] ;
    end
    %return
    mesh = pkg.geometry.mesh.distMesh(args{:},'bndCons',false) ;
    profile off
