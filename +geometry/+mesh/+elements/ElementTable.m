...
methods
    ...
    function M = shares(this,features)
    % Return a sparse matrix of logical values
    % M(i,j) = true if this(j,:) contains features(i,:) 
    %               or if features(i,:) contains this(j,:)
        M = this.contains(features) | features.contains(this)' ;
    end
...
methods
    ...
    % Return a table representing the same mesh but with simplex elements
        ...
        for tt = 1:this.nTypes
            ...
            isType = this.TypeIdx==tt ;
            nodeIdx = elmtTable.dataAtIndices(this.NodeIdx(isType,:)') ;
