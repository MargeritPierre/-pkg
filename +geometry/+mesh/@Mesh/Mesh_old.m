...
methods
    ...
    % Initialize the node position
        this.X = pkg.geometry.mesh.MeshFunction('Mesh',this) ;
    % Process inputs
...
methods
    ...
    % handle values or Mesh Functions
        switch class(values)
            case 'pkg.geometry.mesh.MeshFunction'
                values.Mesh = this ;
                this.X = values ;
            otherwise
...
methods
    ...
    % Assign the mesh
        ...
        else ; mesh = pkg.geometry.mesh.Mesh('X',x,'Elems',elems) ; 
        end
    end
end
...
methods
    ...
    % normals: [nFaces 3]
        ...
        if nargin<2
            E = [0 0 ; this.Faces.Types.centroid] ;
...
methods 
    ...
    % Split the independent parts of a mesh into several-meshes
        ...
        while ~isempty(e2e)
            ...
            meshes(end+1) = pkg.geometry.mesh.Mesh('X',this.X.Values,'Elems',belongElems) ;
    ...
    % Return the new mesh
        mesh = pkg.geometry.mesh.Mesh('X',x,'Elems',elems) ;
    end
...
methods
    ...
    % Edges: we need to retrieve the connectivity...
        ...
        elemEdges = pkg.geometry.mesh.elements.ElementTable('NodeIdx',globalEdgIdx) ;
    % Slice elements
        sliceElems = pkg.geometry.mesh.elements.ElementTable ;
    ...
    % Finally..
        ...
        Xs = [Xn ; Xc] ;
    % Slice mesh
        mesh = pkg.geometry.mesh.Mesh('X',Xs,'Elems',sliceElems) ;
    end
    
    function cut(this,fcn,tol,X)% [meshON,meshIN,meshOUT] = 
    % Return a cut of the mesh so that for any X in mesh, fcn(X)<=0
        if nargin<3 ; tol = this.defaultTolerance ; end
        if nargin<4 ; X = this.X.Values ; end
    % Evaluate the levelset on nodes
        fcnX = fcn(X) ;
    % Get which egdes are crossing the level set
        [fBool,eBool,sBool] = featureLevelSetSign(this,fcnX,this.Elems,tol,X) ;
    ...
    % Edges on the level set: we need to retrieve the connectivity...
        ...
        edgesOnLvlSt = pkg.geometry.mesh.elements.ElementTable('NodeIdx',edgesOnLvlSt) ;
    % Slice elements
        slicedElems = pkg.geometry.mesh.elements.ElementTable ;
        for tt = 1:elemsOnLvlSt.nTypes
            % Retrieve concerned elements
                elmtType = elemsOnLvlSt.Types(tt) ;
                elmtIdx = elemsOnLvlSt.TypeIdx==tt ;
            % Cut the corresponding elements
                output = elmtType.cut(eBool(elmtIdx,1:elmtType.nNodes)) ; % see pkg.geometry.mesh.elements.base.BaseElement.cut
            % From local node indices to global node indices... (tidy part..)
%             if ~isempty(elmtIdx)
%                 % Separate node & edge indices
%                     edgIdx = newElems.NodeIdx.*uint32(newElems.NodeIdx>elmtType.nNodes) ;
%                     nodIdx = newElems.NodeIdx-edgIdx ;
%                 % Go global
%                     nodIdx = elemsOnLvlSt.globalize(nodIdx,elmtIdx) ;
%                     edgIdx = edgesOnLvlSt.globalize(max(edgIdx-elmtType.nNodes,0),elmtIdx) ;
%                     edgIdx(edgIdx>0) = edgIdx(edgIdx>0) + this.nNodes ;
%                 % Assign new elements
%                     newElems.NodeIdx = nodIdx + edgIdx ;
%                 % Add to the list
%                     slicedElems = [slicedElems newElems] ;
%             end
        end
%     % Clean the element list (may be duplicated elements)
%         slicedElems = clean(slicedElems) ;
%     % Unique (and shorter..) list of nodes/edges indices
%         [elmtIdx,nodePos,idx] = find(slicedElems.NodeIdx) ;
%         isNod = idx<=this.nNodes ; isEdg = ~isNod ; % imaginary indices denote edge indices
%         [uNod,na,nc] = unique(idx(isNod)) ;
%         [uEdg,ea,ec] = unique(idx(isEdg)-this.nNodes) ;
%     % Keep nodes
...
%         idx(isEdg) = ec + numel(uNod) ;
%     % Finally..
%         slicedElems.NodeIdx = full(sparse(elmtIdx,nodePos,double(idx))) ;
%         Xs = [Xn ; Xc] ;
%     % Slice mesh
%         mesh = pkg.geometry.mesh.Mesh('X',Xs,'Elems',slicedElems) ;
    end
