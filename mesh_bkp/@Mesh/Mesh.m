...
%% PROPERTIES
    properties
        % The list of nodes [nNodes nCoord] double
        Nodes
        % Following lists contain positive integers (node indices) or NaNs
        Elems % [nElems maxNodesByElem]
    end
    properties (SetAccess = protected)
        % Following lists contain positive integers (node indices) or NaNs
        Faces % [nFaces maxNodesByFace]
        Edges % [nEdges maxNodesByFace]
    end
...
%% COUNT FUNCTIONS
    methods
        % Number of ...
        function N = nNodes(this) ; N = size(this.Nodes,1) ; end
        function N = nEdges(this) ; N = size(this.Edges,1) ; end
        function N = nFaces(this) ; N = size(this.Faces,1) ; end
        function N = nElems(this) ; N = size(this.Elems,1) ; end
        % Other ...
        function N = maxNodesByElem(this) ; N = size(this.Elems,2) ; end
        function N = nNodesInElem(this) ; N = sum(~isnan(this.Elems),2) ; end
    end
...
%% MESH SPACE DIMENSION
    ...
    methods
        ...
        function this = set.nCoord(this,nCoord)
            this.Nodes = [this.Nodes(:,1:min(this.nCoord,nCoord)) zeros(this.nNodes,nCoord-this.nCoord)] ;
        end
        function nCoord = get.nCoord(this) ; nCoord = size(this.Nodes,2) ; end
    end
